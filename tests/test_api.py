...
TEST_UPDATE_MEME_INVALID_DATA = [
    {
        ...
        "info": {"colors": ["white", "black"], "objects": ["smile", "joke"]},
    },
    {
...
def test_read_all_memes(read_all_memes_endpoint, authorized_headers):
    ...
    read_all_memes_endpoint.check_any_mem_in_data()
...
def test_create_meme_without_required_fields(
    create_meme_endpoint, data, authorized_headers
):
    create_meme_endpoint.create_meme(body=data, headers=authorized_headers)
    create_meme_endpoint.check_status_code_is_400()


def test_read_meme(read_meme_endpoint, meme_id, authorized_headers):
    read_meme_endpoint.read_meme(meme_id, headers=authorized_headers)
    read_meme_endpoint.check_status_code_is_200()
    read_meme_endpoint.check_id_is_correct(meme_id)
...
def test_update_meme(update_meme_endpoint, meme_id, authorized_headers):
    ...
    update_meme_endpoint.read_meme(meme_id, headers=authorized_headers)
    update_meme_endpoint.check_info_is_correct(body["info"])
    update_meme_endpoint.check_url_is_correct(body["url"])
    update_meme_endpoint.check_tags_is_correct(body["tags"])
    update_meme_endpoint.check_text_is_correct(body["text"])
    update_meme_endpoint.check_id_is_correct(meme_id)


@pytest.mark.parametrize("data", TEST_UPDATE_MEME_INVALID_DATA)
def test_update_meme_without_required_fields(
    update_meme_endpoint, meme_id, data, authorized_headers
):
...
def test_delete_meme(
    delete_meme_endpoint, read_meme_endpoint, meme_id, authorized_headers
):
    delete_meme_endpoint.delete_meme(meme_id, headers=authorized_headers)
    delete_meme_endpoint.check_status_code_is_200()

    read_meme_endpoint.read_meme(meme_id, headers=authorized_headers)
    read_meme_endpoint.check_meme_not_exist(meme_id, headers=authorized_headers)


def test_delete_meme_with_not_existing_id(
    delete_meme_endpoint, not_existing_id, authorized_headers
):
    delete_meme_endpoint.delete_meme(not_existing_id, headers=authorized_headers)
    delete_meme_endpoint.check_status_code_is_404()


def test_delete_meme_endpoint_with_unauthorized_user(delete_meme_endpoint, meme_id):
    delete_meme_endpoint.delete_meme(meme_id, headers={})
